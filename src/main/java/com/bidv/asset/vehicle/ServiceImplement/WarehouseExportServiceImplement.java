                ...
                v.getLoans().stream()
                    ...
                    .ifPresent(loan -> {
                        ...
                        if (db != null) {
                            ...
                            if (nvl(db.getDisbursementAmount()).compareTo(nvl(db.getTotalAmountPaid())) <= 0) {
...
//        exportEntity.setUpdatedAt(LocalDateTime.now());
    ...
    public List<WarehouseExportDTO> getPendingRequests() {
        ...
        BigDecimal collateralFromDb =
                vehicleRepository.sumPriceByStatus("Giữ trong kho");

        final BigDecimal totalCollateral =
                collateralFromDb != null ? collateralFromDb : BigDecimal.ZERO;

        final BigDecimal realEstate =
                new BigDecimal("8910000000");
